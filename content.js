...
app.encodeResult = function(text) {
    ...
    values.push("&egrave;");
    keys.push(/&Atilde;&ordf;/g);
    values.push("&ecirc;");
    keys.push(/&Atilde;&laquo;/g);
    values.push("&euml;");
    keys.push(/&Atilde;&nbsp;/g);
    values.push("&agrave;");
    keys.push(/&Atilde;&curren;/g);
    values.push("&auml;");
    keys.push(/&Atilde;&cent;/g);
    values.push("&acirc;");
    keys.push(/&Atilde;&sup1;/g);
    values.push("&ugrave;");
    keys.push(/&Atilde;&raquo;/g);
    values.push("&ucirc;");
    keys.push(/&Atilde;&frac14;/g);
    values.push("&uuml;");
    keys.push(/&Atilde;&acute;/g);
    values.push("&ocirc;");
    keys.push(/&Atilde;&para;/g);
    values.push("&ouml;");
    keys.push(/&Atilde;&reg;/g);
    values.push("&icirc;");
    keys.push(/&Atilde;&macr;/g);
    values.push("&iuml;");
    keys.push(/&Atilde;&sect;/g);
    values.push("&ccedil;");
    keys.push(/&amp;/g);
    values.push("&amp;");
...
app.addEntryToDOM = function(parent, entry, pattern) {
    ...
    parent.setAttribute("href", "https://myanimelist.net/" + this.getPageType() + "/" + entry.id);
    ...
    if (app.linkNodes[title] === undefined) {
        ...
        this.getMALInfo(this.getPageType(), title, function(linkInfo) {
            ...
            if (linkInfo.success === true) {
                ...
                // We should avoid triggering many errors in a short time
                // because that gives a very bad impression.
...
app.getANNLinks = function() {
    ...
    for (var i = 0; i < links.length; ++i) {
        var link = links[i];
        var href = link.href;
        if (href.search("https://www.animenewsnetwork.com/encyclopedia/manga") !== -1 || href.search("https://www.animenewsnetwork.com/encyclopedia/anime") !== -1) {
            result.push(href);
        }
    }
    return result;
...
app.addRatingIntoAnimePageDOM = function(linkInfo) {
    ...
    for (var i = 0; i < linkInfo.entries.length; ++i) {
        ...
        td_title.setAttribute("href", "https://myanimelist.net/" + entry.pageType + "/" + entry.id);
...
app.isVisibilityTresholdChanged = function() {
    if (app.visibilityTreshold !== app.getVisibilityTreshold()) {
        ...
        app.trackEvent({
            ...
            action: 'Changed',
            label: "Visibility treshold",
            value: app.visibilityTreshold
        });
        return true;
    }
...
app.isHighlightTresholdChanged = function() {
    if (app.highlightTreshold !== app.getHighlightTreshold()) {
        ...
        app.trackEvent({
            category: 'Configuration',
            action: 'Changed',
            label: "Highlight treshold",
            value: app.highlightTreshold
        });
        return true;
    }
...
try {
    ...
    app.malQueryInfo = {
        "replace": {
            ...
            "Binbō-gami ga!": "Binbougami ga!",
            "Boku no Imōto wa \"Ōsaka Okan\"": "Boku no Imouto wa \"Osaka Okan\"",
            "Cardfight Vanguard": "Cardfight!! Vanguard",
            "Chibi Devi!": "Chibi Devi",
            "Chitose Get You!!": "Chitose Get You",
            "Crayon Shin-chan: The Storm Called!: Me and the Space Princess": "Eiga Crayon Shin-chan: Arashi wo Yobu! Ora to Uchuu no Princess",
            "Doraemon": "Doraemon",
            "Doraemon: Nobita and the Island of Miracles—Animal Adventure": "Doraemon: Nobita and the Miracle Island - Animal Adventure",
            "Dungeon Fighter Online": "Arad Senki: Slap Up Party",
            "Eureka Seven: AO": "Eureka Seven AO",
            "Everyday Mum": "Mainichi Kaasan",
            "The Familiar of Zero": "Familiar of Zero",
            "Fresh Pretty Cure": "Fresh Precure!",
            "GA Geijutsuka Art Design Class": "GA: Geijutsuka Art Design Class",
            "Gen'ei o Kakeru Taiyō": "Genei wo Kakeru Taiyou",
            ...
            "Jinrui wa Suitaishimashita": "Jinrui wa Suitai Shimashita",
            "Koishinasai": "Koi Shinasai",
            "Kon'nichiwa Anne": "Konnichiwa Anne",
            "Kyurem vs. the Sacred Swordsman: Keldeo": "Pokemon Best Wishes!: Kyurem vs. Seikenshi",
            ...
            "Magical Girl Lyrical Nanoha A's": "Magical Girl Lyrical Nanoha",
            ...
            "Metal Fight Beyblade Zero-G": "Metal Fight Beyblade Zero G",
            "Moyasimon: Tales of Agriculture": "Moyashimon",
            ...
            "Niji-Iro Hotaru: Eien no Natsu Yasumi": "Nijiiro Hotaru: Eien no Natsuyasumi",
            "Oh My Goddess": "My Goddess",
            "One Piece Film: Strong World": "One Piece: Strong World",
            "Oniichan dakedo Ai sae Areba Kankeinai yo ne!": "As Long As There's Love",
            ...
            "Persona 4: The Animation": "Persona 4 The Animation",
            ...
            "Starship Troopers: Invasion": "Uchuu no Senshi",
